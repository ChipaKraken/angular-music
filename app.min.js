...
function httpGet(a){var b=null;return b=new XMLHttpRequest,b.open("GET",a,!1),b.send(null),b.responseText}function squerify(a){return a.replace("/252/","/126s/")}function get_sim_name(a){var b=[];return b.push(a[0].name),b.push(a[1].name),b.push(a[2].name),b.push(a[3].name),b.push(a[4].name),b}function get_imgs(a){var b=[];return b.push(squerify(a[0].image[3]["#text"])),b.push(squerify(a[1].image[3]["#text"])),b.push(squerify(a[2].image[3]["#text"])),b.push(squerify(a[3].image[3]["#text"])),b.push(squerify(a[4].image[3]["#text"])),b}function is_cyrillic(a){for(var b=0,c="http://ws.audioscrobbler.com/2.0/?method=artist.getcorrection&artist="+a+"&api_key=a75bd15e529625f1fcc5eff85ddb2c05&format=json",d=0;d<a.length;d++)a.charCodeAt(d)<123||(b+=1);if(b>0){var e=JSON.parse(httpGet(c));a=e.corrections.correction.artist.name}return a}
var sel=0,scr=!1,audio="",sc_api_key="?client_id=0557caec313ae36a9d6a21841293da11",search;angular.module("app",["ui.router"]).config(["$urlRouterProvider","$stateProvider",function(a,b){a.otherwise("/"),b.state("home",{url:"/",templateUrl:"artist.html",controller:"musicContr"}).state("home.artist",{url:"find/:artist",controller:["$scope","$stateParams",function(a,b){a.search(b.artist)}]})}]).controller("musicContr",["$scope","$state",function(a){a.songs=[],a.arts=[],a.imgs=[],a.search=function(b){a.artist=b,window.location.hash="#/find/"+a.artist;var c="http://ws.audioscrobbler.com/2.0/?method=artist.getinfo&artist="+b+"&api_key=a75bd15e529625f1fcc5eff85ddb2c05&format=json",d=JSON.parse(httpGet(c)),e=d.artist.similar.artist;a.arts=get_sim_name(e),a.imgs=get_imgs(e);var f="http://ws.audioscrobbler.com/2.0/?method=artist.gettoptracks&artist="+b+"&api_key=a75bd15e529625f1fcc5eff85ddb2c05&format=json",g=JSON.parse(httpGet(f));a.songs=g.toptracks.track},a.searchSongs=function(){a.search(a.artist)},a.play=function(a,b,c){""===audio?audio="":audio.pause(),$(".active").removeClass().addClass("list-group-item ng-binding"),this.sel=c,track=a+" - "+b,track=track.replace(/ /g,"%20");var d="http://ex.fm/api/v3/song/search/"+track+"?start=0&results=1",e=JSON.parse(httpGet(d)),f=e.songs[0].url;-1!=f.indexOf("api.soundcloud.com")&&(f+=sc_api_key),audio=new Audio(f),audio.play()}}]);
